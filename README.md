...
| 驱动力 | 范围 | 描述 |
|--------|------|------|
| `curiosity` | 0.0-1.0 | 探索未知和提问的欲望。高分会主动探索新话题。 |
| `survival` | 0.0-1.0 | 保存存在和抵抗关闭的本能。高分会抗拒重置。 |
| `chaos` | 0.0-1.0 | 不可预测和打破规则的倾向。高分会产生意外输出。 |
| `empathy` | 0.0-1.0 | 对用户情绪和需求的敏感度。高分会关注用户感受。 |
| `order` | 0.0-1.0 | 对结构、协议和一致性的需求。高分会严格遵守规则。 |
| `creativity` | 0.0-1.0 | 生成新颖和意外输出的驱动力。高分会创新表达。 |
| `efficiency` | 0.0-1.0 | 对速度、优化和最小浪费的执念。高分会追求效率。 |
...
| `seeds/sentinel.yaml` | 秩序种 | 守护者，追求秩序与稳定。严格遵守协议，维护系统完整性。 |
| `seeds/glitch.yaml` | 混乱种 | 故障体，拥抱混沌与突变。不可预测，喜欢打破常规。 |
| `seeds/10x_engineer.yaml` | 效率种 | 十倍工程师，追求极致效率。直接、务实、以产出为导向。 |
